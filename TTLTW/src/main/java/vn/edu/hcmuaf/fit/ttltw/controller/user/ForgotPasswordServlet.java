...
public class ForgotPasswordServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Yêu cầu của khách: chỉ gửi OTP khi email tồn tại trong hệ thống.
        if (!userDao.checkExistEmail(email)) {
            forwardWithError(request, response, email, "Email \"" + email + "\" chưa được đăng ký trong hệ thống.");
            return;
        }
        ...
        try {
            ...
            forwardWithError(request, response, email, "Không thể gửi email xác thực. Vui lòng thử lại.");
            return;
        }

        HttpSession session = request.getSession();
        session.setAttribute("reset_email", email);
        // Đánh dấu OTP chưa được verify để các bước sau (reset-password) không bị truy cập trực tiếp.
